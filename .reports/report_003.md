...
`orders/order_003.md` に基づき，論文執筆用 Figure Set を生成し，研究内容・実験方法・実験結果をまとめたレポートである．  
...
**例外**: 相関係数ヒートマップ（Fig. 1）のみカラーとし，$r=1.0$ を赤，$r=0$ を白，$r=-1.0$ を青とする（`RdBu_r` カラーマップ）．

Feature Importance 棒グラフ（Fig. 2）は 3 目的変数間で横軸上限を 1.0 に統一する．
...
| 図番 | ファイル | 内容 |
| :--- | :--- | :--- |
| Fig. 1 | `figures/correlation_input_target.png` | 説明変数と目的変数の Pearson 相関係数（カラー） |
| Table 1 | `tables/cv_within_group.md` | CV と郡内標準偏差 |
| Table 2 | `tables/model_comparison.md` | 3 モデルの Train/Test RMSE |
| Fig. 2a–c | `figures/feature_importance_*.png` | Random Forest F.I.（横軸 0–1.0 統一） |
| Fig. 3a–c | `figures/boxplot_*.png` | 最重要特徴量と目的変数の Box-Plot |
...
Random Forest の F.I.（fold 平均）の結果は以下の通りである．3 図とも横軸を 0–1.0 に統一している．
...
F.I. 上位特徴量（3 目的変数すべて Discharge tube）の各水準における目的変数分布を Box-Plot で示した．

- **MB conc. vs Discharge tube**: 0.7 m で中央値が高く，2 m 以上で低い分布
- **UFB conc. vs Discharge tube**: 水準間で広いばらつき，単調な増減ではない
- **Oxygen cont. vs Discharge tube**: 0.7 m で低く，2 m 以上で高い分布
